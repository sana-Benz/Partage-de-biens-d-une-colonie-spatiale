...
public class Colon {
     ...
     * boolean result= this.preferences.indexOf(ressource) <
     * this.preferences.indexOf(this.ressourceAttribuée);
    ...
    public boolean prefereObjet(Ressource ressource) {
        ...
        if (!this.preferences.contains(ressource)) {
            System.out.println(
                    "Erreur : la ressource " + ressource + " n'est pas trouvée dans les préférences de " + nom);
            System.out.println("préference du colon" + nom + ": " + this.preferences);
            return false;
        }
        if (!this.preferences.contains(this.ressourceAttribuee)) {
            System.out.println("Erreur : la ressource attribuée " + this.ressourceAttribuee
                    + " n'est pas trouvée dans les préférences de " + nom);
            return false;
        }
        boolean result = this.preferences.indexOf(ressource) < this.preferences.indexOf(this.ressourceAttribuee);
        System.out.println(
                "Le colon " + nom + " préfère " + ressource + " à " + this.ressourceAttribuee + " ? " + result);
        return result;
    }
